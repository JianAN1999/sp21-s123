...
public class Model extends Observable {
    ...
    public boolean tilt(Side side) {
        ...
        for (int col = 0; col < board.size(); col++) {
            for (int row = board.size() - 1; row >= 0; row--) {
                ...
                int next_row = nextNonNullTileRow(col, row);
                if (next_row == -1) {
                    break;
                }
                Tile next_t = board.tile(col, next_row);
                if (t == null || t.value() == next_t.value()) {
                    changed = true;
                    if (board.move(col, row, next_t)) {
                        score += 2 * t.value();
                    } else {
                        row += 1;
                    }
    ...
    private int nextNonNullTileRow(int col, int row) {
        for (int cur_row = row - 1; cur_row >= 0; cur_row--) {
            if (board.tile(col, cur_row) != null) {
                return cur_row;
            }
        }
        return -1;
